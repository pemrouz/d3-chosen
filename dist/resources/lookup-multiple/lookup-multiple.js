...
function lookupMultiple(state) {
  ...
  }).classed('is-selected', is.in(value)).on('click.select', th(function (el) {
    return function (e) {
      return toggleOption(datum(el));
    };
  })).on('mouseover.suggestion', changeSuggestion).html(state.renderer).each(scrollIntoViewIfNeeded);
  ...
  function backspaceLozenge(d, i, el, e) {
    ...
    var focusOffset = _dos$getSelection.focusOffset;
    var anchorNode = _dos$getSelection.anchorNode;
    var focusNode = _dos$getSelection.focusNode;
  ...
  function toggleOption(d) {
    is.in(value)(d) ? value.splice(value.indexOf(d), 1) : value.push(d);
